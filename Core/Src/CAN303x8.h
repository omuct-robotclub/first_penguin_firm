...
class CAN_303x8 {
 ...
 public:
  HAL_StatusTypeDef
  send(uint32_t ID, ID_type ide, Frame_type rtr, uint8_t *data, uint32_t data_len);  //send data to CAN bus
  HAL_StatusTypeDef subscribe_message(uint32_t ID, ID_type ide, Frame_type rtr,
                                      FIFO fifo);  //subscribe to a message to put message in the FIFO
